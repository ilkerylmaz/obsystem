...
import { getTeacherInfoById, getTeacherDashboardStats } from "@/app/services/teacherService";
...
export default function TeacherDashboard() {
    ...
    const [stats, setStats] = useState<any>({ activeCourseCount: 0, totalStudents: 0, courses: [] });

    useEffect(() => {
        if (!userId) return;
        getTeacherInfoById(userId).then((res) => {
            if (res.data) setInfos(res.data);
            else setInfos(res);
        })
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <StatCard title={t.dashboard.teacher.totalCourses} value={stats.activeCourseCount || 0} icon={<BookIcon />} color="blue" description="Açılan aktif dersler" />
            ...
            <div className="obs-card">
                ...
                <div className="divide-y divide-[var(--surface-border)]">
                    {!stats.courses || stats.courses.length === 0 ? (
                        <p className="p-5 text-sm text-[var(--text-muted)]">Herhangi bir açılan ders bulunamadı.</p>
                    ) : (
                        stats.courses.map((c: any) => {
                            const quota = c.quota || 1;
                            const pct = Math.round((c.enrolledStudentCount / quota) * 100);
                            return (
                                <div key={c.teacherLessonId} className="px-5 py-4">
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <Badge variant="info" size="sm">{c.semesterName}</Badge>
                                            <span className="text-sm font-bold text-[var(--text-primary)]">{c.enrolledStudentCount} öğrenci</span>
                                        </div>
                                    </div>
                                    {/* Progress bar */}
                                    ...
                                    <p className="text-xs text-[var(--text-muted)] mt-1">{pct}% toplam içinde (Kontenjan: {c.quota || '-'})</p>
                                </div>
                            );
                        })
                    )}
